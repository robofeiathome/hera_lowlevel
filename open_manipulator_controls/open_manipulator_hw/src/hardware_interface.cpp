...
void HardwareInterface::registerControlInterfaces()
{
  ...
  {
    std::string tailString = iter->first.substr((iter->first.length() >= 6 ? iter->first.length()-6 : 0));
    if (tailString == "_slave")
    {
      continue;
    }
...
void HardwareInterface::write()
{
  ...
  if (strcmp(interface_.c_str(), "position") == 0)
  {
    for (auto const& dxl:dynamixel_)
    {
      std::string tailString = dxl.first.substr((dxl.first.length() >= 6 ? dxl.first.length()-6 : 0));
      if (tailString == "_slave")
      {
        // ROS_ERROR("%u", (uint8_t)dxl.second);
        // ROS_ERROR("%s", dxl.first.c_str());
        // ROS_ERROR("%f", joints_[(uint8_t)dxl.second-1].position_command);
        // ROS_ERROR("%f", joints_[(uint8_t)dxl.second-1].position);
        // id_array[id_cnt] = 0;
        // dynamixel_position[id_cnt] = 0;
        // id_cnt ++;
        continue;
      }
      id_array[id_cnt] = (uint8_t)dxl.second;
